...
/// Registry: resolve a style id to its effect. Unknown ids fall back to `bar`.
pub fn effect_for(style: &str) -> &'static dyn WaveEffect {
    match style {
        "line"   => &effects::line::LineEffect,
        "mirror" => &effects::mirror::MirrorEffect,
        "dot"    => &effects::dot::DotEffect,
        "neon"   => &effects::neon::NeonEffect,
        "orb"    => &effects::orb::OrbEffect,
        "pulse"  => &effects::pulse::PulseEffect,
        "eq"     => &effects::eq::EqEffect,
        _        => &effects::bar::BarEffect, // default = "bar"
    }
}

/// Draw the waveform for one frame.
///
/// Signature is unchanged from the pre-refactor monolith so callers
/// (`render_frame`) need no edits. Builds a [`WaveCtx`] and dispatches to the
/// effect resolved by [`effect_for`].
