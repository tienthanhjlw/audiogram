  ...
  const render = async () => {
    ...
    onChange({
      ...
      logs: [
        'Starting render…',
        captionsPath
          ? `Subtitles${state.karaokeEnabled ? ' (karaoke)' : ''}: ${captionsPath}`
          : '(no subtitles)',
      ],
    })
    ...
    try {
      ...
      onChange({ logs: [...state.logs, `Error: ${e?.message ?? String(e)}`], isRendering: false })
    }
  }
      ...
      <div style={{ width: 300, display: 'flex', flexDirection: 'column', gap: 12, flexShrink: 0 }}>
        ...
        <div style={{
          ...
          {[
            ...
            { label: 'Format',     value: state.canvasSize },
