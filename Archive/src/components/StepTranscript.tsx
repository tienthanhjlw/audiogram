...
import { AppState, Segment, ModelInfo, CANVAS_SIZES, LayoutTemplate } from '../types'
...
// ── Layout subtitle Y defaults ────────────────────────────────────────────────

const LAYOUT_DEFAULT_SUBTITLE_Y: Partial<Record<LayoutTemplate, number>> = {
  spotify: 0.85,
  minimal: 0.74,
  fullbg:  0.83,
  split:   0.80,
}
function getDefaultYPct(layout: LayoutTemplate): number | null {
  return LAYOUT_DEFAULT_SUBTITLE_Y[layout] ?? null
}
...
export default function StepTranscript({ state, onChange, onBack, onNext }: Props) {
  ...
  const defaultYPct    = getDefaultYPct(state.layoutTemplate)
  const effectiveYPct  = state.subtitleYPct ?? defaultYPct
  const canDrag        = defaultYPct !== null && hasSubtitles

  // Active segment — whichever contains currentTime
  const activeIdx = useMemo(() => {
    if (!state.segments.length) return -1
    return state.segments.findIndex(s => currentTime >= s.start && currentTime < s.end)
  }, [state.segments, currentTime])

  const activeSeg = activeIdx >= 0 ? state.segments[activeIdx] : undefined

  // Auto-scroll Virtuoso to keep active segment visible
  useEffect(() => {
  ...
  const runTranscribe = useCallback(async () => {
    ...
    try {
      const rawSegs = await invoke<Segment[]>('transcribe_audio', {
        ...
        modelName: state.whisperModel,
      })
      const segs = splitSegments(rawSegs, state.peaks, 3.5)
  ...
  }, [state.audioPath, state.whisperModel, state.peaks, onChange])

  const handleTranscribeClick = () => {
    if (state.segments.length > 0) setConfirmRetranscribe(true)
    else runTranscribe()
  }
  ...
  const updateSegment = useCallback((id: number, text: string) => {
    ...
    invoke<string>('write_srt', { segments: updated }).then(p => onChange({ srtPath: p })).catch(() => {})
  ...
  const computeYPct = (clientY: number) => {
    const el = canvasWrapRef.current
    if (!el) return effectiveYPct ?? 0.74
    const rect = el.getBoundingClientRect()
    return Math.max(0.10, Math.min(0.95, (clientY - rect.top) / rect.height))
  }
  const onPointerDown = (e: React.PointerEvent) => {
    if (!canDrag) return
    draggingRef.current = true
    ;(e.target as HTMLElement).setPointerCapture(e.pointerId)
    onChange({ subtitleYPct: computeYPct(e.clientY) })
  }
  ...
  const audioSrc = state.audioPath ? convertFileSrc(state.audioPath) : ''
      ...
      <div style={{ width: 340, display: 'flex', flexDirection: 'column', gap: 12, flexShrink: 0 }}>
        ...
        <div style={{
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
            ...
            {models.map(m => {
              const isSelected    = state.whisperModel === m.name
              ...
              return (
                <div
                  key={m.name}
                  onClick={() => m.downloaded && onChange({ whisperModel: m.name })}
                  style={{
          ...
          {confirmRetranscribe && (
            <div style={{
              ...
              <div style={{ color: '#B45309', marginBottom: 10 }}>
                {state.segments.length} segments sẽ bị thay thế.
              </div>
        ...
        {state.segments.length > 0 && (
          <div style={{
            ...
            <div style={{ fontSize: 11, fontWeight: 600, color: '#6B7280', textTransform: 'uppercase', letterSpacing: '0.05em' }}>
              {state.segments.length} Segments — click để sửa / seek
            </div>
            <Virtuoso
              ref={virtuosoRef}
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: 14, minWidth: 0 }}>
        ...
        <div style={{
          ...
          <RowToggle
            ...
            sub={state.segments.length === 0 ? 'Nhận dạng audio trước' : undefined}
              ...
              <div style={{ display: 'flex', gap: 6, alignItems: 'center', flexWrap: 'wrap' }}>
                {SUBTITLE_COLORS.map(c => (
                  <button key={c.hex} title={c.name}
                    onClick={() => hasSubtitles && onChange({ subtitleColor: c.hex })}
                    style={{
                      width: 26, height: 26, borderRadius: 13,
                      background: c.hex, border: '1.5px solid #E5E7EB', flexShrink: 0,
                      cursor: hasSubtitles ? 'pointer' : 'default',
                      boxShadow: state.subtitleColor === c.hex ? '0 0 0 2px #fff, 0 0 0 4px #6C4FF6' : '0 1px 3px rgba(0,0,0,0.12)',
                      transition: 'box-shadow 0.15s',
                    }}
                  />
                ))}
                <input type="color" value={state.subtitleColor} disabled={!hasSubtitles}
                ...
                {KARAOKE_COLORS.map(c => (
                  <button key={c.hex} title={c.name}
                    ...
                    style={{
                      ...
                      boxShadow: state.karaokeColor === c.hex ? `0 0 0 2px #fff, 0 0 0 4px ${c.hex}` : '0 1px 3px rgba(0,0,0,0.18)',
                      transition: 'box-shadow 0.15s',
                    }}
                  />
                ))}
                <input type="color" value={state.karaokeColor}
        ...
        <div style={{
          ...
          {canDrag && (
            <div style={{
              ...
              background: 'rgba(108,79,246,0.85)', backdropFilter: 'blur(6px)',
          ...
          >
            ...
            {canDrag && effectiveYPct !== null && (
              <div style={{
                position: 'absolute', left: 0, right: 0,
                top: `${effectiveYPct * 100}%`,
                transform: 'translateY(-50%)',
                pointerEvents: 'none', zIndex: 5,
              }}>
                <div style={{
                  height: 2,
                  backgroundImage: 'repeating-linear-gradient(to right, rgba(255,255,255,0.7) 0px, rgba(255,255,255,0.7) 8px, transparent 8px, transparent 14px)',
                  boxShadow: '0 0 6px rgba(108,79,246,0.8)',
                }} />
                <div style={{
                  position: 'absolute', top: '50%', left: '50%',
                  transform: 'translate(-50%, -50%)',
                  background: '#6C4FF6', borderRadius: 12, padding: '3px 10px',
                  display: 'flex', alignItems: 'center', gap: 4,
                  boxShadow: '0 2px 8px rgba(0,0,0,0.5)',
                }}>
                  <GripIcon />
                  <span style={{ fontSize: 10, color: '#fff', fontWeight: 600, lineHeight: 1 }}>
                    {Math.round(effectiveYPct * 100)}%
                  </span>
                </div>
              </div>
            )}
          ...
          <span>
            {hasSubtitles
              ? state.karaokeEnabled
                ? `Karaoke · ${state.karaokeColor.toUpperCase()} · chữ ${state.subtitleColor.toUpperCase()}`
                : `${state.segments.length} segments · chữ ${state.subtitleColor.toUpperCase()}`
              : 'Preview layout'}
          </span>
          {state.subtitleYPct !== null && canDrag && (
            <button
              onClick={() => onChange({ subtitleYPct: null })}
              style={{ background: 'none', border: 'none', cursor: 'pointer', padding: '2px 8px', fontSize: 11, color: '#6C4FF6', fontFamily: 'inherit', fontWeight: 600 }}
            >
              Reset vị trí
            </button>
          )}
