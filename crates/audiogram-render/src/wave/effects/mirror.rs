...
impl WaveEffect for MirrorEffect {
    fn id(&self) -> &'static str { "mirror" }
    ...
    fn draw(&self, px: &mut [u8], c: &WaveCtx) {
        ...
        for (i, &p) in c.heights.iter().enumerate() {
            let half = (p * c.wh / 2.0).max(3.0);
            let cx = (c.wx as f32 + i as f32 * (bar_w + gap) + bar_w * 0.5) as i32;
            let y0 = (mid_y - half).max(0.0) as i32;
            let y1 = (mid_y + half).min(c.h as f32) as i32;
            let half_w = (bar_w * 0.5).max(1.0) as i32;
            draw_capsule_bar(px, c.w, c.h, cx, y0, y1, half_w, c.wc, 0.19, 1.0);
        }
    }
}
